...
const resolveWebUIPort = (config: WebUIUserConfig): number => {
  ...
  const envPort = parsePortValue(process.env.MARGAY_PORT ?? process.env.PORT, 'environment variable (MARGAY_PORT/PORT)');
...
const resolveRemoteAccess = (config: WebUIUserConfig): boolean => {
  const envRemote = parseBooleanEnv(process.env.MARGAY_ALLOW_REMOTE || process.env.MARGAY_REMOTE);
  const hostHint = process.env.MARGAY_HOST?.trim();
