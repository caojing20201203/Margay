# Margay Database System

本文档介绍 Margay 的新数据库系统，它使用 **better-sqlite3** (主进程) 作为持久化存储。
...
- **数据库文件**: `{userData}/config/margay.db`
- **图片文件**: `{userData}/data/images/`

其中 `{userData}` 为：

- macOS: `~/Library/Application Support/Margay/`
- Windows: `%APPDATA%/Margay/`
- Linux: `~/.config/Margay/`
...
直接复制 `margay.db` 和 `margay.db-wal` 文件即可。
